...
import {ReactionGrid,NumberSprint,MemoryMatch} from './components/QuickGames';
...
import Top30Update from './components/common/Top30Update';
...
const pagePath:Record<Page,string>={
  Home:'/',Launchpad:'/launchpad',Apps:'/apps',Arcade:'/games',Multiplayer:'/multiplayer','Retro Vault':'/retro-vault',Play:'/play',
  BrightPath:'/brightpath',CodeCoach:'/codecoach',ProjectForge:'/projectforge','Device Lab':'/device-lab',Community:'/community',
  CareerQuest:'/careerquest',Portfolio:'/portfolio','School Tools':'/school-tools','Creative Lab':'/creative-lab','Labs Store':'/store',Developer:'/developer',Settings:'/settings'
};
const pathPage=new Map(Object.entries(pagePath).map(([page,path])=>[path,page as Page]));
const pageFromPath=():Page=>pathPage.get(window.location.pathname.replace(/\/$/,'')||'/')||'Home';

async function askMGLabsAI(message:string,system:string){
  const res=await fetch('/api/ai',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message,system})});
...
function Home({go}:{go:(p:Page)=>void}){
  const [recent,setRecent]=useState<RecentItem[]>(()=>safeRead<RecentItem[]>('mg-recent',[]).slice(0,6));
  ...
  return <div className="page">
    ...
    <section className="section">
      <div className="section-head"><div><span>CONTINUE</span><h2>Recent activity</h2></div><button onClick={()=>{safeWrite('mg-recent',[]);setRecent([])}}>Clear</button></div>
      <div className="grid">
...
function Play(){
  return <div className="page"><h1>MGLabs Originals</h1><p>Playable games included directly in Version 22. Scores and state stay in your browser.</p><MetroRush/><VoxelFrontier/><ReactionGrid/><NumberSprint/><MemoryMatch/></div>
}

function BrightPath(){
  const [mode,setMode]=useState('Explain');const [subject,setSubject]=useState('General');
  const [t,setT]=useState('');const [busy,setBusy]=useState(false);const [m,setM]=useState<{role:'user'|'ai',text:string}[]>([{role:'ai',text:'BrightPath is ready. Pick a mode and subject, then ask a question.'}]);
  ...
  const send=async()=>{if(!t.trim()||busy)return;const question=t.trim();setT('');setM(v=>[...v,{role:'user',text:question}]);setBusy(true);try{const system=`You are BrightPath inside MGLabs. Help a student with ${subject}. Mode: ${mode}. Be accurate, clear, age-appropriate, and teach instead of just dumping answers.`;const reply=await askMGLabsAI(question,system);setM(v=>[...v,{role:'ai',text:reply}])}catch(err:any){setM(v=>[...v,{role:'ai',text:`AI connection needs attention: ${err.message}. If this is a new Cloudflare deployment, enable the Workers AI binding named AI.`}])}finally{setBusy(false)}};
  return <div className="page"><h1>BrightPath</h1><p>Real AI tutoring through the MGLabs Cloudflare AI endpoint when the AI binding is enabled.</p><div className="tools"><select value={mode} onChange={e=>setMode(e.target.value)}>{modes.map(x=><option key={x}>{x}</option>)}</select><select value={subject} onChange={e=>setSubject(e.target.value)}>{subjects.map(x=><option key={x}>{x}</option>)}</select></div><div className="quick-row">{['Simplify','Give example','Make harder','Turn into notes','Create practice'].map(x=><button onClick={()=>setT(x+': ')} key={x}>{x}</button>)}</div><div className="chat">{m.map((x,i)=><div className={x.role==='user'?'user':'ai'} key={i}>{x.text}</div>)}{busy&&<div className="ai">Thinking…</div>}</div><div className="compose"><input value={t} onChange={e=>setT(e.target.value)} onKeyDown={e=>e.key==='Enter'&&send()} placeholder="Ask BrightPath..."/><button disabled={busy} onClick={send}>{busy?'Working…':'Send'}</button></div></div>
}

function CodeCoach(){
  const [code,setCode]=useState('<h1>Hello MGLabs</h1>\n<p>Edit this HTML.</p>');
  const [out,setOut]=useState(code);const [aiHelp,setAiHelp]=useState('');const [busy,setBusy]=useState(false);
  ...
  const explain=async()=>{setBusy(true);try{setAiHelp(await askMGLabsAI(`Review this browser code and explain one improvement at a time:\n\n${code}`,'You are CodeCoach inside MGLabs. Give safe, beginner-friendly coding guidance. Do not execute code.'))}catch(e:any){setAiHelp(`AI connection needs attention: ${e.message}`)}finally{setBusy(false)}};
  return <div className="page"><h1>CodeCoach</h1><p>Frontend playground, coding challenges, and AI code explanations.</p><div className="split"><div><div className="section-head"><h2>HTML Playground</h2><div className="buttons"><button onClick={()=>setOut(code)}>Run</button><button className="secondary" onClick={explain}>{busy?'Thinking…':'Ask CodeCoach AI'}</button></div></div><textarea className="code-editor" value={code} onChange={e=>setCode(e.target.value)}/>{aiHelp&&<div className="ai-help">{aiHelp}</div>}</div><div><h2>Preview</h2><iframe className="preview-frame" sandbox="allow-scripts" srcDoc={out} title="preview"/></div></div><section className="section"><h2>Challenge ladder</h2><div className="grid">{challenges.map((x,i)=><article className="card" key={x}><span>LEVEL {i+1}</span><h3>{x}</h3><p>Use the playground and AI coach to work through this challenge.</p><button onClick={()=>setCode(`<!-- ${x} -->\n`)}>Start challenge</button></article>)}</div></section></div>
}
...
function DeviceLab(){
  ...
  const generate=()=>{setN(n+1);setHistory(h=>[{...packet,id:n+1},...h].slice(0,50))}
  const exportCsv=()=>{if(!history.length)return;const rows=['packet,temp_c,humidity_percent,light,pressure_hpa,co2_ppm',...history.map(x=>[x.id,x.temp,x.humidity,x.light,x.pressure,x.co2].join(','))];const blob=new Blob([rows.join('\n')],{type:'text/csv'});const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download='mglabs-device-lab.csv';a.click();URL.revokeObjectURL(url)}
  return <div className="page"><h1>Device Lab</h1><p><b>SIMULATION MODE.</b> Telemetry is generated locally and does not claim a physical hardware connection.</p><div className="buttons"><button onClick={generate}>Generate packet</button><button className="secondary" disabled={!history.length} onClick={exportCsv}>Export CSV</button><button className="secondary" onClick={()=>setHistory([])}>Clear history</button></div><div className="metrics">{[['Temperature',packet.temp+'°C'],['Humidity',packet.humidity+'%'],['Light',packet.light],['Pressure',packet.pressure+' hPa'],['CO₂-equivalent',packet.co2+' ppm'],['Packets',n]].map(x=><article key={x[0]}><span>{x[0]}</span><b>{x[1]}</b></article>)}</div><section className="section"><h2>Recent telemetry</h2><div className="table">{history.map(x=><div key={x.id}><b>Packet {x.id}</b><span>{x.temp}°C</span><span>{x.humidity}% RH</span><span>{x.co2} ppm</span></div>)}</div></section></div>
}
...
function CreativeLab(){
  const [name,setName]=useState('MGLabs');const [purpose,setPurpose]=useState('Student AI + technology platform');const [output,setOutput]=useState('');
  const make=(kind:string)=>{const ideas:Record<string,string>={'Poster plan':`${name} poster: bold title, one-sentence purpose, 3 feature cards, one strong screenshot, and a call-to-action.`,'Video script':`Hook: Meet ${name}. Explain the problem, show ${purpose}, demo 3 features, and end with the main benefit.`,'Thumbnail plan':`${name} thumbnail: large title, one product screenshot, one strong icon, minimal text, high contrast.`,'UI wireframe':`${name} wireframe: header + search, hero, quick actions, featured apps, recent activity, content grid, footer.`};setOutput(ideas[kind]||'')}
  return <div className="page"><h1>Creative Lab</h1><p>Quick creative planning workspace.</p><div className="form-grid"><label><span>PROJECT NAME</span><input value={name} onChange={e=>setName(e.target.value)}/></label><label><span>PURPOSE</span><textarea value={purpose} onChange={e=>setPurpose(e.target.value)}/></label></div><article className="creative-preview"><span>BRAND CONCEPT</span><h2>{name}</h2><p>{purpose}</p><div className="buttons">{['Poster plan','Video script','Thumbnail plan','UI wireframe'].map(x=><button key={x} onClick={()=>make(x)}>{x}</button>)}</div>{output&&<div className="ai-help">{output}</div>}</article></div>
}
...
function Settings(){
  ...
  useEffect(()=>{applyTheme(theme);document.body.classList.toggle('compact',compact)},[]);
  return <div className="page"><h1>Settings</h1><p>Personalize MGLabs while keeping the same dark-tech visual system.</p><div className="settings-list"><label><span>Theme</span><select value={theme} onChange={e=>applyTheme(e.target.value)}>{themes.map(x=><option key={x}>{x}</option>)}</select></label><label><span>Compact cards</span><input type="checkbox" checked={compact} onChange={e=>{setCompact(e.target.checked);localStorage.setItem('mg-compact',e.target.checked?'1':'0');document.body.classList.toggle('compact',e.target.checked)}}/></label><button onClick={()=>{if(confirm('Clear local MGLabs data?')){localStorage.clear();location.reload()}}}>Clear local data</button></div></div>
}

export default function App(){
  const [p,setP]=useState<Page>(()=>pageFromPath());
  const go=(x:Page)=>{setP(x);const path=pagePath[x];if(window.location.pathname!==path)window.history.pushState({},'',path);scrollTo({top:0,behavior:'smooth'})};
  useEffect(()=>{const onPop=()=>setP(pageFromPath());window.addEventListener('popstate',onPop);return()=>window.removeEventListener('popstate',onPop)},[]);
  ...
  return <>
    ...
    <main>
      {p==='Home'&&<Home go={go}/>} {p==='Launchpad'&&<Launchpad go={go}/>} {p==='Apps'&&<Apps/>}
      {p==='Arcade'&&<Arcade go={go}/>} {p==='Multiplayer'&&<Multiplayer/>} {p==='Retro Vault'&&<RetroVault/>} {p==='Play'&&<Play/>}
      {p==='BrightPath'&&<BrightPath/>} {p==='CodeCoach'&&<CodeCoach/>} {p==='ProjectForge'&&<ProjectForge/>} {p==='Device Lab'&&<DeviceLab/>}
      {p==='Community'&&<Community/>} {p==='CareerQuest'&&<CareerQuest/>} {p==='Portfolio'&&<Portfolio/>} {p==='School Tools'&&<SchoolTools/>}
      {p==='Creative Lab'&&<CreativeLab/>} {p==='Labs Store'&&<LabsStore/>} {p==='Developer'&&<Developer/>} {p==='Settings'&&<Settings/>}
    </main>
    <footer>MGLabs Version 22 · Cloudflare-ready · Built / Prototype / Planned / External status system</footer>
  </>
}
